...
local function sort_class_sname()
	...
	for idx, section in ipairs(section_list) do
		...
		m.uci:set(qos_config, check_sname, "upload_class")
...
local function sort_rule_sname()
	...
	m.uci:foreach(qos_config, "upload_rule", function(s)
		-- 记录sname不对的部分
		check_idx = check_idx + 1
		check_sname= "upload_rule_" .. (check_idx * 100)
		if s[".name"] ~= nil and s[".name"] ~= check_sname then
			-- 添加乱序的section
			table.insert(section_list, s)
			table.insert(section_sname, check_sname)
	...
	if is_unorder == true then
		...
		for idx, section in ipairs(section_list) do
			check_sname = section_sname[idx]
			m.uci:set(qos_config, check_sname, "upload_rule")
			for key, value in pairs(section) do
...
function TypedSection.remove(self, section)
	...
	if string.find(section, "upload_rule_") then
		-- 通知修改排序
		mes_rule_sort = true
		-- 通知是删除
...
function class_s.create(self, section)
	...
	mes_rule_sort = false
	-- 计算class总数
	uci:foreach(qos_config, "upload_class", function(_)
		class_idx = class_idx + 1
	end)
	-- 不管有没有都要+1,这个+1是给创建新的规则条用的
	class_idx = class_idx + 1
	class_sname = "uclass_%d" % class_idx
	uci:set(qos_config, class_sname, "upload_class")
...
function rule_s.create(self, section)
		...
		mes_rule_sort = false
		-- 计算section总个数
		m.uci:foreach(qos_config, "upload_rule", function(_)
			rule_idx = rule_idx + 1
		end)
		-- 不管有没有都要+1
		rule_idx = rule_idx + 1
		rule_order = "%d" % (rule_idx * 100)
		rule_name = "upload_rule_" .. (rule_idx * 100)
		m.uci:set(qos_config, rule_name, "upload_rule")
